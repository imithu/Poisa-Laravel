...
class Paypal
{
     ...
     * @param string $history_mode    // '' | 'topup'
     * 
     * @since   1.0.0
     * @version 1.5.0
     * @author  Mahmudul Hasan Mithu
     */
    public static function transaction_add( int $user_id, object $transaction_details, string $history_mode='' )
    {
    ...
    public static function transaction_verify( int $user_id )
    {
      ...
      foreach( $transaction_all as $transaction ){
        ...
        if($transaction_is_valid===true){
          ...
          if($transaction->history_mode==='topup'){
            Topup_History::insert( $user_id, $transaction->PayPal_purchase_amount, 'paypal_personal' );
          }
        }else{
          DB::table('Poisa_Transaction_Checkout_Paypal')
            ->where('id', $transaction->id )
